...
class MediaIndexViewTestCase(BaseTestCase):
    def test_upload_shows_up_on_index_page(self):
        ...
        self.assertContains(response, self.f1.file_url)
        self.assertNotContains(response, self.f2.file_url)
...
class OembedDetailViewTestCase(BaseTestCase):
    ...
    def test_get_request_returns_required_data_for_embedly_for_each_file_type(self):
        ...
        self.assertContains(response, '"url": "%(url)s"' % {"url": self.f1.file_url})
        ...
        self.assertContains(response2, '"url": "%(url)s"' % {"url": self.f4.file_url})
